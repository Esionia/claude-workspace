...
1. vocabulary-card场景（必须使用定义模式）：
   - 展示：单词 + 词性 + 中文释义（definitions 数组，3-4条，**必须全部使用中文**）
   - ❌ 不要展示中文释义（CoverScene 封面已展示过）
   - 不要生成 phraseAudioUrl、exampleAudioUrl、phraseAudioDuration 等音频字段
2. highlight场景：必须讲解词根词缀、构词法或记忆方法
3. example场景（必须2个，每个必须包含 highlights 数组）：
   - **highlights 数组必须填写！** 不能为空数组或 undefined
   - 格式：`highlights: [{"text": "目标单词", "color": "#FFD93D"}]`
   - 注意：`text` 必须是目标单词本身（如 "resilience"），且必须能在英文例句中找到
...
## 输出 JSON 结构
{
  ...
  "scenes": [
    ...
    {"id": "scene_002", "type": "vocabulary-card", "content": {"type":"vocabulary-card","phrase":"单词","partOfSpeech":"adj.","definitions":["中文释义1","中文释义2","中文释义3"]}, "narration": "中文旁白", "narrationLang": "zh", "durationSeconds": 15},
...
"""
        ...
        try:
            ...
            content = re.sub(r'""([^"]+)""', r"'\1'", content)
            # 修复单引号问题：智能转换，把Python dict格式的单引号转为JSON双引号
            # 只在外层引号位置转换，字符串内部的双引号需要转义
            def fix_quotes(text):
                result = []
                i = 0
                while i < len(text):
    ...
    async def generate_audio_for_scene(self, scene: dict, word: str) -> dict:
        ...
        if scene_type == "example" and content.get("english"):
            ...
            # highlights 兜底：如果 LLM 没有生成 highlights，自动从英文句子中提取目标单词高亮
            if not content.get("highlights") or len(content.get("highlights", [])) == 0:
                # 从英文句子中查找目标单词（不区分大小写）
                lower_english = english_text.lower()
                lower_word = word.lower()
                if lower_word in lower_english:
                    # 找到目标单词，提取原文大小写版本
                    idx = lower_english.find(lower_word)
                    actual_word = english_text[idx:idx+len(word)]
                    content["highlights"] = [{"text": actual_word, "color": "#FFD93D"}]
                    print(f"   ⚠️ 自动补充 highlights: \"{actual_word}\"")
