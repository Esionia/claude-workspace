...
2. vocabulary-card - 展示单词卡片（词性 + 中文释义列表，不含中文释义，因为封面已展示）
3. highlight - 词根词缀讲解
4. example - 例句演示（需要2个例句）
5. summary - 要点总结

场景必须按照上述 1→2→3→4→5 的顺序生成，不得调换顺序！注意：不再生成 title 场景！
...
4. example场景（必须2个，每个必须包含 highlights 数组）：
...
## 输出 JSON 结构
{
  ...
  "scenes": [
    ...
    {"id": "scene_002", "type": "vocabulary-card", "content": {"type":"vocabulary-card","phrase":"单词","partOfSpeech":"adj.","definitions":["释义1","释义2","释义3"]}, "narration": "中文旁白", "narrationLang": "zh", "durationSeconds": 15},
